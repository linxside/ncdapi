...
help() {
	...
	echo "-M   MOD Record	  ncdapi.sh -M ID HOST DOMAIN RECORDTYPE DESTINATION [PRIORITY]"
	echo "-D   DEL Record	  ncdapi.sh -D ID HOST DOMAIN RECORDTYPE DESTINATION [PRIORITY]"
	echo "-g   get all Records	ncdapi.sh -g DOMAIN"
	echo "-b   backup from Zone	ncdapi.sh -b DOMAIN"
	echo "-R   Restore Zone	ncdapi.sh -R FILE"
	echo "-s   get SOA    	ncdapi.sh -s DOMAIN"
	echo "-S   change SOA    	ncdapi.sh -S DOMAIN TTL REFRESH RETRY EXPIRE DNSSECSTATUS"
